...
async def subscribe_to_updates(call: types.CallbackQuery) -> None:
    ...
    product_code: Optional[str] = await query_manager.get_latest_query_by_user_id(user_id)
    if product_code:
        ...
        test_task.delay()
        logging.info(f"User: {username}, ID: {user_id} successfully subscribed to notifications!")
    else:
        logging.info(f"Subscription denied for user: {username}, ID: {user_id}, no product code in db!")
